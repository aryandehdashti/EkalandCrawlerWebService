...
def productParser(productUrl):
    if productUrl is not None:
        ...
        if len(rawProduct.findAll(string=re.compile("متاسفانه کالا در حال حاضر موجود نیست."))) == 0:
            products = []
...
def findProduct(productName):
    ...
    rawSearchResult = soup.findAll('a')[1].get('href') if len(soup.findAll('a')) > 0 else None
    return productParser(BASE_URL+rawSearchResult)
